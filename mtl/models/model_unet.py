...
from mtl.models.model_parts import Encoder, get_encoder_channel_counts, ASPP, DecoderDeeplabV3p, \
    SelfAttention, DecoderTransposed, DecoderUnet


class ModelUnet(torch.nn.Module):
    def __init__(self, cfg, outputs_desc):
        ...
        print('USING UNET MODEL :DDDDDDDDDDDDDDD')

        ch_out_encoder_bottleneck, ch_out_encoder_4x = get_encoder_channel_counts(cfg.model_encoder_name)
        # ch_out_encoder_4x = 512
        print('ch out encoder 4x: ', ch_out_encoder_4x)
        ch_out_encoder_2x = 64
        ch_out_encoder_8x = 128

        self.aspp_semseg = ASPP(ch_out_encoder_bottleneck, 256)
        self.decoder_semseg = DecoderUnet(256, ch_out_encoder_8x, ch_out - 1)

        self.assp_depth = ASPP(ch_out_encoder_bottleneck, 256)
        self.decoder_depth = DecoderUnet(256, ch_out_encoder_8x, 1)

        self.sa_from_depth2semseg = SelfAttention(128, 128)
        self.sa_from_semseg2depth = SelfAttention(128, 128)

        self.decoder_semseg2 = DecoderTransposed(128, ch_out - 1)
        self.decoder_depth2 = DecoderTransposed(128, 1)
    ...
    def forward(self, x):
        ...
        intermediate_predictions_semseg, features_before_final_layer_semseg = self.decoder_semseg(features_tasks_semseg,
                                                                                                  features[8], features[4], features[2])
        ...
        # Decoder Depth
        intermediate_predictions_depth, features_before_final_layer_depth = self.decoder_depth(features_tasks_depth, features[8], features[4], features[2])
        ...
        del intermediate_predictions_depth
        del intermediate_predictions_semseg
