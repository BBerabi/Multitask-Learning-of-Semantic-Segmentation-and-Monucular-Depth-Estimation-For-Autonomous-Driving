...
from mtl.models.model_parts import Encoder, get_encoder_channel_counts, ASPP, DecoderDeeplabV3p, \
    SelfAttention, DecoderDistillation
...
class ModelDistillation(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder Semseg
        intermediate_predictions_4x_semseg, features_before_final_layer_semseg = self.decoder_semseg(features_tasks_semseg, features[4])
        # Intermediate predictions for semseg
        intermediate_predictions_1x_semseg = F.interpolate(intermediate_predictions_4x_semseg, size=input_resolution,
                                                           mode='bilinear', align_corners=False)
        # ASSP Depth
        features_tasks_depth = self.assp_depth(features_lowest)
        # Decoder Depth
        intermediate_predictions_4x_depth, features_before_final_layer_depth = self.decoder_depth(features_tasks_depth, features[4])
        # Intermediate predictions for depth
        intermediate_predictions_1x_depth = F.interpolate(intermediate_predictions_4x_depth, size=input_resolution,
                                                          mode='bilinear',
                                                          align_corners=False)

        intermediate_predictions_1x = torch.cat((intermediate_predictions_1x_semseg, intermediate_predictions_1x_depth),
                                                1)
        ...
        final_predictions_4x_semseg = self.decoder_semseg2(features_before_final_layer_semseg, attention_from_depth2semseg)
        final_predictions_4x_depth = self.decoder_depth2(features_before_final_layer_depth, attention_from_semseg2depth)

        final_predictions_1x_semseg = F.interpolate(final_predictions_4x_semseg, size=input_resolution, mode='bilinear',
                                              align_corners=False)
        final_predictions_1x_depth = F.interpolate(final_predictions_4x_depth, size=input_resolution, mode='bilinear', align_corners=False)

        final_predictions_1x = torch.cat((final_predictions_1x_semseg, final_predictions_1x_depth), 1)

        out = {}
        offset = 0
        for task, num_ch in self.outputs_desc.items():
            out[task] = [intermediate_predictions_1x[:, offset:offset + num_ch, :, :],
                         final_predictions_1x[:, offset:offset + num_ch, :, :]]
            offset += num_ch
